...
contract ButtonwoodPair is IButtonwoodPair, ButtonwoodERC20 {
    ...
    function _getNewStoredBalances(
        ...
        returns (
            uint256 newPool,
            uint256 newReservoir,
            uint256 newOtherPool,
            uint256 newOtherReservoir
        )
    {
        newPool = pool;
    ...
    function _syncReservoirs(
        ...
        (newPool0, newReservoir0, newPool1, newReservoir1) = _getNewStoredBalances(
            balance0,
            newPool0,
            newPool1,
            newReservoir0,
        ...
        (newPool1, newReservoir1, newPool0, newReservoir0) = _getNewStoredBalances(
            balance1,
            newPool1,
            newPool0,
            newReservoir1,
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            liquidity =
                _totalSupply.mul(amount1.add(amount1)) /
                (_pool1.add(_pool1).add(reservoir0InTermfOf1).add(_reservoir1));
        }
    ...
    function mintWithReservoir(address to) external lock returns (uint256 liquidity) {
...

        {
            ...
            if (amount0 > 0) {
                ...
                liquidity = _totalSupply.mul(newPool1.mul(amount0)) / (newPool1.add(newPool1).add(newReservoir1).mul(newPool0));
                ...
                // liquidity is a proportion of value added over existing pool value
                liquidity = _totalSupply.mul(newPool0.mul(amount1)) / (newPool0.add(newPool0).add(newReservoir0).mul(newPool1));
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, 'Buttonwood: INSUFFICIENT_INPUT_AMOUNT');
        {
            ...
            require(
                balance0Adjusted.mul(balance1Adjusted) >= uint256(_pool0).mul(_pool1).mul(1000**2),
                'Buttonwood: K'
            );
        }
